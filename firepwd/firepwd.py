...
def decryptMoz3DES( globalSalt, masterPassword, entrySalt, encryptedData, verbose ):
...
def getLoginData(directory, verbose):
  logins = []
...
def extractSecretKey(masterPassword, keyData, verbose): #3DES
  #see http://www.drh-consultancy.demon.co.uk/key3.html
...
def decryptPBE(decodedItem, masterPassword, globalSalt, verbose):
...
def getKey( masterPassword, directory, verbose ):  
  if (directory / 'key4.db').exists():
    ...
    if clearText == b'password-check\x02\x02': 
      ...
      if a102 == CKA_ID: 
        ...
        #decrypt master key
        clearText, algo = decryptPBE( decoded_a11, masterPassword, globalSalt, verbose )
